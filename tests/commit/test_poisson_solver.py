...
with math.precision(64):
    ...
    def compare(reference, dics, plot=True, fnc_list=[np.min, np.mean, np.max]):
        ...
        heads = " | ".join(
            [f"{'name':<10}"] + [f"{fnc.__name__:>10}" for fnc in fnc_list]
        )
        descs = {
    ...
    class TestPoissonSolvers(TestCase):
        def test_poisson(self):
            ...
            solver_dict = {
                "FFT_solve": lambda x: domain.grid(
                    FFT_solve(x.values, dx)
                ).values.numpy(order="z,y,x")[0],
            ...
            math.assert_close(
                reference,
                solver_soln["FFT_solve"],
                rel_tolerance=1,
                abs_tolerance=1e-14,
            )
            math.assert_close(
                reference,
                solver_soln["CG_solve"],
                rel_tolerance=1,
                abs_tolerance=1e-14,
            )
