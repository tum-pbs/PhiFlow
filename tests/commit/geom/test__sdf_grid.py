...
from phi.geom import Box, Sphere, sample_sdf, SDFGrid
...
class TestSDFGrid(TestCase):
    ...
    def test_sdf_from_geometry(self):
        ...
        sdf = sample_sdf(sphere, Box(x=3, y=3), x=100, y=100)
    ...
    def test_sdf_creation(self):
        sphere = Sphere(x=1, y=1, radius=.8)
        sdf1 = sample_sdf(sphere, Box(x=3, y=3), x=100, y=100)
    ...
    def test_signed_distance(self):
        ...
        sdf = sample_sdf(sphere, Box(x=(2, 3), y=3), x=100, y=100)  # signed_distance inside sphere only approximate
    ...
    def test_sdf_rebuild(self):
        ...
        bounds = Box(x=3, y=2)
        sdf = sample_sdf(spheres, bounds, x=64, y=64, rebuild='from-surface')
