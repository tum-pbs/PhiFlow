                ...
                with backend:
                    ...
                    with math.record_gradients(velocity.values):
                        for _ in range(4):
                            ...
                            velocity, _ = fluid.make_incompressible(velocity, DOMAIN, solve=Solve('CG-adaptive', 1e-5, 0))
                ...
                with backend:
                    ...
                    def simulate(velocity: StaggeredGrid, smoke: CenteredGrid):
                        for _ in range(4):
                            ...
                            buoyancy_force = smoke * (0, 0.5) >> velocity
                            velocity = advect.semi_lagrangian(velocity, velocity, dt=1) + buoyancy_force
                            velocity, _ = fluid.make_incompressible(velocity, DOMAIN, solve=Solve('CG-adaptive', 1e-5, 0, suppress=(NotConverged, Diverged)))
                        loss = field.l2_loss(
