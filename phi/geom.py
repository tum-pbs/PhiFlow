...
class Grid(struct.Struct):
    ...
    def shape(self, components=1, batch_size=1, name=None):
        return field.CenteredGrid(name, self.box, tensor_shape(batch_size, self._resolution, components), batch_size=batch_size)

    def staggered_shape(self, batch_size=1, name=None):
        return field.StaggeredGrid(name, self.box, tensor_shape(batch_size, self._resolution + 1, self.rank), batch_size=batch_size)
