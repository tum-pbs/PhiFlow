...
class TestNetworks(TestCase):
    ...
    def test_u_net_2d_network_sizes(self):
        for lib in LIBRARIES:
            net = lib.u_net(2, 3, batch_norm=False, activation='SiLU', in_spatial=2, use_res_blocks=True)
            self.assertEqual(39059, lib.parameter_count(net))

    def test_u_net_3d_norm_network_sizes(self):
        for lib in LIBRARIES:
            net = lib.u_net(2, 3, batch_norm=True, activation='SiLU', in_spatial=3, use_res_blocks=True)
            self.assertEqual(113939, lib.parameter_count(net))

    def test_u_net_1d_norm_network_sizes(self):
        for lib in LIBRARIES:
            net = lib.u_net(2, 3, batch_norm=True, activation='SiLU', in_spatial=1, use_res_blocks=True)
            self.assertEqual(14867, lib.parameter_count(net))

    def test_optimize_u_net(self):
        for lib in LIBRARIES:
            net = lib.u_net(1, 1, levels=2, use_res_blocks=True, activation='SiLU')
...
test_obj = TestNetworks()
test_obj.test_u_net_1d_norm_network_sizes()
test_obj.test_u_net_2d_network_sizes()
test_obj.test_u_net_3d_norm_network_sizes()
test_obj.test_optimize_u_net()
test_obj.test_optimize_u_net_jit()
